...
function BlockSubTask({ subtask, onChangeStatus }: BlockSubTaskProps) {
    return (
        <div className="task-item__subtask">
            ...
            {subtask.description && (
                <MarkDown
                    className="task-item__subtask-description"
                    markdown={subtask.description}
                />
            )}
            {subtask.motivation && (
                <MarkDown
                    className="task-item__subtask-motivation"
                    markdown={subtask.motivation}
                />
            )}
            {subtask.instruction && (
                <MarkDown
                    className="task-item__subtask-instruction"
                    markdown={subtask.instruction}
                />
            )}
            {subtask.continuance && 0 < subtask.continuance ? (
                <div className="task-item__subtask-continuance">
                    {subtask.continuance} hour
                    {1 < subtask.continuance ? "s" : ""}
                </div>
            ) : null}
        </div>
    );
