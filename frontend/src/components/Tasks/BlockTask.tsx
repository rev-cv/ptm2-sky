...
function Task({ objTask, index }: TaskProps) {
    ...
    const deadline = objTask.deadline
        ? new Date(objTask.deadline).getTime()
        : null;
    const deadlaneDiff = deadline
        ? Math.floor((deadline + dayms - Date.now()) / dayms)
        : null;
    const deadlineClass =
        deadlaneDiff != null && deadlaneDiff < 1
            ? "day_1"
            : deadlaneDiff && deadlaneDiff < 3
              ? "days_3"
              : deadlaneDiff && deadlaneDiff < 7
                ? "week_1"
                : deadlaneDiff && deadlaneDiff < 14
                  ? "week_2"
                  : "in_near_future";
    ...
    const [doneSubTasksCount /*waitSubTasksCount*/] = objTask.subtasks.reduce(
        (prev, cur) =>
            cur.status ? [prev[0] + 1, prev[0]] : [prev[0], prev[1] + 1],
        [0, 0],
    );

    function getRiskImpactValue(arg1: string, arg2: number) {
        if (arg1 === "risk") {
            const riskValue = riskImpact?.risk.find(
                (item) => item.value === arg2,
            );
            return riskValue ? riskValue : null;
        } else if (arg1 === "impact") {
            const impactValue = riskImpact?.impact.find(
                (item) => item.value === arg2,
            );
        ...
        <>
            ...
            >
                ...
                {0 < objTask.description.trim().length ? (
                    <MarkDown
                        className="task-item__descr"
                        markdown={objTask.description}
                    />
                ) : null}
                    ...
                    <>
                        <div className={`task-item__subtasks-btn`}>
                            ...
                            >
                                ...
                                {objTask.subtasks.length < 2
                                    ? " step"
                                    : " steps"}
                            </button>
                        </div>
                        <div
                            className={`task-item__subtask-extender${isOpenSubTasks ? " view" : ""}`}
                        >
                            <div>
                                {objTask.subtasks
                ...
                <div className={`task-item__annexe`}>
                    ...
                    {(objTask.activation || objTask.deadline) && (
                        <div className="task-item__annexe__dates">
                            ...
                            {objTask.deadline && (
                                <div
                                    className={`task-item__annexe__deadline ${deadlineClass}`}
                                >
                                    <IcoStart />
                                    {deadline ? (
                                        <span>
