...
export async function wsCommander(
    ...
    try {
        const res = await fetchAuth(`${APIURL}/api/get_ws_token`, {
            method: "GET",
        });
        if (!res.ok) {
            const errText = await res.text();
            throw new Error(
                `Ошибка получения токена для websocket: ${errText}`,
            );
        }
    ...
    return new Promise<TypeViewTask | undefined>((resolve, reject) => {
        ...
        ws.onmessage = (event) => {
            ...
            if (response.status === G_Status.ADDED) {
                ...
                response.status === G_Status.COMPLETED &&
                response.data
            ) {
                ...
                store.set(atomGenTaskBuffer, (prev) => ({
                    ...prev,
                    [task.id]: { ...bufferElement, ...response.data },
                }));
...
function updateGen(command: CommandValues) {
    // сигнализация об окончании генерации
    switch (command) {
        ...
        case Commands.GEN_INTENSITY:
            store.set(atomGenIntensity, (prev) => ({ ...prev, isGen: false }));
            break;
        default:
            break;
    }
...
function resetGen(command: CommandValues) {
    // сигнализация о сбросе запущенной генерации
    switch (command) {
        case Commands.GEN_MOTIVE:
        ...
        case Commands.GEN_STEPS:
            ...
            break;
        case Commands.GEN_RISK:
            store.set(atomGenRisk, {
                fixed: { risk: 0, risk_explanation: "", risk_proposals: "" },
                isGen: false,
            });
            break;
        case Commands.GEN_THEME:
        ...
        case Commands.GEN_ACTION:
            ...
            break;
        case Commands.GEN_INTENSITY:
            store.set(atomGenIntensity, { fixed: {}, isGen: false });
            break;
        default:
            break;
    }
